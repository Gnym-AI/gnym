...
func TestCoordinatorRunReviewerErrorStopsExecution(t *testing.T) {
	for _, failingIndex := range []int{0, 1} {
		t.Run(string(rune('0'+failingIndex)), func(t *testing.T) {
			wantErr := errors.New("review failed")
			diff := Diff{Content: "the diff"}
			configs := []reviewer.Config{{Name: "first"}, {Name: "second"}, {Name: "not-called"}}
			errorsByCall := make([]error, failingIndex+1)
			errorsByCall[failingIndex] = wantErr
			diffReviewer := &fakeReviewer{
				results: []reviewer.Payload{{Summary: "First completed"}},
				errors:  errorsByCall,
			}
			sink := &fakeCommentSink{}
			coordinator := NewCoordinator(&fakeDiffSource{diff: diff}, diffReviewer, configs, sink)

			err := coordinator.Run()
			if !errors.Is(err, wantErr) || !strings.Contains(err.Error(), configs[failingIndex].Name) {
				t.Errorf("Run() error = %v, want reviewer %q error", err, configs[failingIndex].Name)
			}
			if len(diffReviewer.calls) != failingIndex+1 {
				t.Errorf("Reviewer.Review() calls = %d, want %d", len(diffReviewer.calls), failingIndex+1)
			}
			if len(sink.runs) != 0 {
				t.Errorf("CommentSink.Save() calls = %d, want 0", len(sink.runs))
			}
